...
www_dir=/var/www
domain=laravel-datagrid.tatrytec.eu
www_new_app_dir=$www_dir/deploy-new-$domain
www_old_app_dir=$www_dir/deploy-old-$domain


# git clone is in the pipeline gonfiguration
#git clone https://github.com/camohub/$domain.git $www_new_app_dir
cd $www_new_app_dir
echo "---------------------------------------------------"
echo " git clone done "
echo "---------------------------------------------------"

cp $www_dir/$domain/.env $www_new_app_dir
mkdir -p $www_new_app_dir/storage/framework/
cp -R $www_dir/$domain/storage/framework/sessions/ $www_new_app_dir/storage/framework/
cp -R $www_dir/$domain/storage/app/ $www_new_app_dir/storage/
echo "---------------------------------------------------"
...
mv $www_dir/$domain $www_old_app_dir
echo "---------------------------------------------------"
echo " old app folder rename done "
echo "---------------------------------------------------"

mv $www_new_app_dir $www_dir/$domain
echo "---------------------------------------------------"
